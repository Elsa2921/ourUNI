...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(
        isset($post['idToken'])
        and isset($post['googleAuth'])
    ){
        googleAuth_($post['idToken']);
    }
    elseif(
        isset($post['form'])
        and !empty($post['form'])
    ){
        if($post['form']==='signup_n'){
            if((isset($post['name']) and isset($post['surname']))){
                if(!empty($post['name']) and !empty($post['surname'])){
                    signup1($post['name'],$post['surname']);
                }
                else{
                    http_response_code(403);
                    echo json_encode(["error" => "Invalid filed!"]);
                    exit();
                }
            }
            else{
        ...
        elseif($post['form']==='email_form'){
            if(isset($post['email']) and !empty($post['email'])){
                signup_email($post['email']);
            }
            else{
                http_response_code(401);
                echo json_encode(["error" => "Invalid filed!"]);
        ...
        elseif($post['form']=='auth_form'){
            if(isset($post['code']) and !empty($post['code'])){
                authChecker($post['code']);
            }
        }
        elseif($post['form']=='password_form'){
            if(isset($post['password']) and !empty($post['password'])){
                passwordAdder($post['password']);
            }
        }
        elseif($post['form']=='forgotPass_form'){
            if(isset($post['email']) and !empty($post['email'])){
